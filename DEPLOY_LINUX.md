...
每份备份包含同一时间戳的 `.sql.gz` 数据库文件、`.images.tar.gz` 资产图片文件，以及覆盖两者的 `.sql.gz.sha256` 校验文件。资产图片保存在独立的 `asset_images` 卷，必须与数据库成套保存、迁移和恢复；只有数据库的旧备份不能恢复文件图片。头像仍在数据库中。

恢复前停止业务写入，先验证校验文件，再恢复 SQL，并把图片归档中的 `asset-images/` 解压到后端 `/app/data/`（对应图片卷）。不要仅恢复数据库。当前脚本是在线备份，要求备份期间不要进行资产或图片变更；需要严格一致的迁移备份时，应先安排维护窗口阻止业务写入，完成备份后再开放。脚本不会擅自停止生产服务。

后端镜像为新图片卷预设 `asset` 用户的写入权限。既有卷的属主不会随镜像重建改变；如原有卷由 root 创建，应在维护窗口检查并修正该卷权限。资产编辑接口现要求携带读取结果中的 `version`，以 `If-Match` 请求头提交；前后端需同步发布，旧页面需刷新。
